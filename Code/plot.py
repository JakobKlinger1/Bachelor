...
def create_diagrams(y_true, y_pred,y_conf, title):
    ...
    fig.savefig(title + ".png", format="png", dpi=144, bbox_inches="tight", pad_inches=0.2)
    plt.close()

def create_histogramm(data1, data2, dataset1, dataset2, batchsize, title, xlabel):
    plt.hist(data1, 50, alpha=0.5, color = 'g', label=dataset1, density=True)
    plt.hist(data2, 50, alpha=0.5, color = 'r', label=dataset2, density=True)
    plt.xlabel(xlabel)
    plt.legend()
    plt.show()
    plt.savefig(title +"histogram.png", format="png", bbox_inches="tight", pad_inches=0.2)
    plt.close()
    
def draw_roc(tpr, fpr, d1, d2, bs, title, auroc): 
    plt.plot(fpr, tpr)
    plt.xlabel("FPR")
    plt.ylabel("TPR")
    plt.text(0.6, 0.2, 'AUROC: ' + str(auroc), fontsize=10)
    plt.show()
    plt.savefig(title +"roc.png", format="png", bbox_inches="tight", pad_inches=0.2)
    plt.close()